...
class Command(BaseCommand):
    ...
    NEWSLETTER_VALUES = [
        ...
        re.compile(".*newletter.*"),
        re.compile("^newsleter.*"),
        re.compile("^local newsletter.*"),
    ]
    NEWSPAPER_VALUES = [re.compile("^newspaper.*"), re.compile("^newpaper.*")]
    ...
    def clean_style_value(self, value):
        ...
        if self._is_match(value, self.NEWSLETTER_VALUES):
            return "Newsletter"
        if self._is_match(value, self.NA_VALUES) or self._is_match(
            value, self.NO_VALUES
        ):
            return "n/a"
    ...
    def handle(self, **options):
        ...
        for line in csv.DictReader(open(csv_path)):
            ...
            if self.clean_value(line["Leader mentioned"]):
                _add_question(
                    "has_leader", self.clean_value(line["Leader mentioned"])
                )

            if self.clean_style_value(
                line[
                    "Leaflet Style (Leaflet, Letter, Magazine, Newsletter, Newspaper, CV or Survey)"
                ]
            ):
                _add_question(
                    "leaflet_style",
                    self.clean_style_value(
                        line[
                            "Leaflet Style (Leaflet, Letter, Magazine, Newsletter, Newspaper, CV or Survey)"
                        ]
                    ),
            ...
            if self.clean_value(line["Leader Picture included"]):
                _add_question(
                    "has_leader_photo",
                    self.clean_value(line["Leader Picture included"]),
                )
            ...
            if self.clean_value(line["Opposition leader pictured"]):
                _add_question(
                    ...
                    self.clean_value(line["Opposition leader pictured"]),
                )

            if self.clean_value(line["Opposition leader mentioned"]):
                _add_question(
                    "has_opposition_leader",
                    self.clean_value(line["Opposition leader mentioned"]),
                )

            if self.clean_value(line["Does it include a graph?"]):
                _add_question(
                    "include_graph",
                    self.clean_value(line["Does it include a graph?"]),
                )

            if self.clean_value(line["""squeeze message"""]):
                _add_question(
                    "squeeze_message",
                    self.clean_value(line["""squeeze message"""]),
                )
